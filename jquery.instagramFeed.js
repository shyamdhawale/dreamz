                ...
                if (options.display_gallery) {
                    if (typeof data.is_private !== "undefined" && data.is_private === true) {
                        ...
                        for (var i = 0; i < max; i++) {
                            ...
                            // html += '<img/>'
                            // html += '<div class="caption_text">' + caption + '</div>'
                            // html += '<img class="instagram_profile_image" src="' + data.profile_pic_url  + '" alt="'+ (is_tag ? data.name + ' tag pic' : data.username + ' profile pic') + '"' + styles.profile_image + (options.lazy_load ? ' loading="lazy"' : '') + ' />';
                            ...
                            updatePhoto = function(){
                                ...
                                var url = "https://www.instagram.com/p/" + imgs[photoIndex].node.shortcode,
                                image, type_resource, 
                                caption = escape_string(parse_caption(imgs[photoIndex], data));

                                switch (imgs[photoIndex].node.__typename) {
                                    case "GraphSidecar":
                                        type_resource = "sidecar"
                                        image = imgs[photoIndex].node.thumbnail_resources[image_index].src;
                                        break;
                                    case "GraphVideo":
                                        type_resource = "video";
                                        image = imgs[photoIndex].node.thumbnail_src
                                        break;
                                    default:
                                        type_resource = "image";
                                        image = imgs[photoIndex].node.thumbnail_resources[image_index].src;
                                ...
                                // $('img').fadeOut(2000, function(){
                                    var myImage = $('#instaImage');
                                    var myCaption = $('.caption_text');
                                    myImage.fadeOut(3000, function(){
                                        myImage.attr('src', image);
                                        
                                        myImage.fadeIn(5000);
                                    });
                                    myCaption.fadeOut(3000, function(){
                                        
                                        myCaption.html(caption);
                                        myCaption.fadeIn(5000);
                                    })
                                
                                
                                photoIndex++
                                setTimeout(function(){updatePhoto()}, 10000);
                            }
                            // html += '</a>';
                        }
                        html += '</div>';
